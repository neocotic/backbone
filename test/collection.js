(function() {
  ...
  module("Hipbone.Collection", {

    setup: function() {
      a         = new Hipbone.Model({id: 3, label: 'a'});
      b         = new Hipbone.Model({id: 2, label: 'b'});
      c         = new Hipbone.Model({id: 1, label: 'c'});
      d         = new Hipbone.Model({id: 0, label: 'd'});
      e         = null;
      col       = new Hipbone.Collection([a,b,c,d]);
      otherCol  = new Hipbone.Collection();
    }
  ...
  test("String comparator.", 1, function() {
    var collection = new Hipbone.Collection([
      {id: 3},
      {id: 1},
      {id: 2}
    ], {comparator: 'id'});
  ...
  test("new and parse", 3, function() {
    var Collection = Hipbone.Collection.extend({
      parse : function(data) {
  ...
  test("clone preserves model and comparator", 3, function() {
    var Model = Hipbone.Model.extend();
    var comparator = function(model){ return model.id; };

    var collection = new Hipbone.Collection([{id: 1}], {
      model: Model,
  ...
  test("get with non-default ids", 5, function() {
    var MongoModel = Hipbone.Model.extend({idAttribute: '_id'});
    var model = new MongoModel({_id: 100});
    var col = new Hipbone.Collection([model], {model: MongoModel});
    equal(col.get(100), model);
    equal(col.get(model.cid), model);
    equal(col.get(model), model);
    equal(col.get(101), void 0);

    var col2 = new Hipbone.Collection();
  ...
  test('get with "undefined" id', function() {
    var collection = new Hipbone.Collection([{id: 1}, {id: 'undefined'}]);
    equal(collection.get(1).id, 1);
  });

  test("update index when id changes", 4, function() {
    var col = new Hipbone.Collection();
    col.add([
      {id : 0, name : 'one'},
      {id : 1, name : 'two'}
    ]);
  ...
  test("add", 14, function() {
    ...
    e = new Hipbone.Model({id: 10, label : 'e'});
    otherCol.add(e);
    ...
    var f = new Hipbone.Model({id: 20, label : 'f'});
    var g = new Hipbone.Model({id: 21, label : 'g'});
    var h = new Hipbone.Model({id: 22, label : 'h'});
    var atCol = new Hipbone.Collection([f, g, h]);
    ...
    var coll = new Hipbone.Collection(new Array(2));
    var addCount = 0;
  ...
  test("add multiple models", 6, function() {
    var col = new Hipbone.Collection([{at: 0}, {at: 1}, {at: 9}]);
  ...
  test("add; at should have preference over comparator", 1, function() {
    var Col = Hipbone.Collection.extend({
  ...
  test("add model to multiple collections", 10, function() {
    var counter = 0;
    var e = new Hipbone.Model({id: 10, label : 'e'});
    ...
    var colE = new Hipbone.Collection([]);
    colE.on('add', function(model, collection) {
      equal(e, model);
      equal(colE, collection);
    });
    var colF = new Hipbone.Collection([]);
  ...
  test("add model with parse", 1, function() {
    ...
    var Col = Hipbone.Collection.extend({model: Model});
    var col = new Col;
  ...
  test("add model to collection with sort()-style comparator", 3, function() {
    ...
    var tom = new Hipbone.Model({name: 'Tom'});
    var rob = new Hipbone.Model({name: 'Rob'});
    var tim = new Hipbone.Model({name: 'Tim'});
    col.add(tom);
    col.add(rob);
    col.add(tim);
  ...
  test("add and remove return values", 13, function() {
    ...
    equal(list.length, 2);
    ok(list[0] instanceof Hipbone.Model);
  ...
  test("shift and pop", 2, function() {
    var col = new Hipbone.Collection([{a: 'a'}, {b: 'b'}, {c: 'c'}]);
  ...
  test("events are unbound on remove", 3, function() {
    var counter = 0;
    var dj = new Hipbone.Model();
    var emcees = new Hipbone.Collection([dj]);
  ...
  test("remove in multiple collections", 7, function() {
    ...
    var passed = false;
    var e = new Hipbone.Model(modelData);
    var f = new Hipbone.Model(modelData);
    f.on('remove', function() {
      passed = true;
    });
    var colE = new Hipbone.Collection([e]);
    var colF = new Hipbone.Collection([f]);
    ok(e != f);
    ok(colE.length === 1);
    ok(colF.length === 1);
    colE.remove(e);
  ...
  test("remove same model in multiple collection", 16, function() {
    var counter = 0;
    var e = new Hipbone.Model({id: 5, title: 'Othello'});
    ...
    var colF = new Hipbone.Collection([e]);
  ...
  test("Colllection: non-persisted model destroy removes from all collections", 3, function() {
    var e = new Hipbone.Model({title: 'Othello'});
  ...
  test("jashkenas/backbone#3283 - fetch with an error response calls error with context", 1, function () {
    var collection = new Hipbone.Collection();
    var obj = {};
    var options = {
      context: obj,
  ...
  test("create with validate:true enforces validation", 3, function() {
    var ValidatingModel = Hipbone.Model.extend({
      validate: function(attrs) {
        return "fail";
      }
    });
    var ValidatingCollection = Hipbone.Collection.extend({
  ...
  test("create will pass extra options to success callback", 1, function () {
    var Model = Hipbone.Model.extend({
      sync: function (method, model, options) {
        ...
        return Hipbone.Model.prototype.sync.call(this, method, model, options);
      }
    });
  ...
  test("where and findWhere", 8, function() {
    var model = new Hipbone.Model({a: 1});
    var coll = new Hipbone.Collection([
      model,
      {a: 1},
      {a: 1, b: 2},
      {a: 2, b: 2},
  ...
  test("Underscore methods with object-style and property-style iteratee", 22, function () {
    var model = new Hipbone.Model({a: 4, b: 1, e: 3});
    var coll = new Hipbone.Collection([
      {a: 1, b: 1},
      {a: 2, b: 1, c: 1},
      {a: 3, b: 1},
  ...
  test ("reset with different values", function(){
    var col = new Hipbone.Collection({id: 1});
    col.reset({id: 1, a: 1});
    equal(col.get(1).get('a'), 1);
  });

  test("same references in reset", function() {
    var model = new Hipbone.Model({id: 1});
    var collection = new Hipbone.Collection({id: 1});
  ...
  test("reset passes caller options", 3, function() {
    ...
    var col = new (Hipbone.Collection.extend({ model: Model }))();
  ...
  test("add does not alter arguments", 2, function(){
    var attrs = {};
    var models = [attrs];
    new Hipbone.Collection().add(models);
    equal(models.length, 1);
    ok(attrs === models[0]);
  });

  test("jashkenas/backbone#714: access `model.collection` in a brand new model.", 2, function() {
    var collection = new Hipbone.Collection;
    collection.url = '/test';
    var Model = Hipbone.Model.extend({
      set: function(attrs) {
  ...
  test("jashkenas/backbone#574, remove its own reference to the .models array.", 2, function() {
    var col = new Hipbone.Collection([
  ...
  test("jashkenas/backbone#861, adding models to a collection which do not pass validation, with validate:true", 2, function() {
    var Model = Hipbone.Model.extend({
      validate: function(attrs) {
  ...
  test("Invalid models are discarded with validate:true.", 5, function() {
    ...
    collection.model = Hipbone.Model.extend({
  ...
  test("jashkenas/backbone#964 - collection.get return inconsistent", 2, function() {
    var c = new Hipbone.Collection();
    ok(c.get(null) === undefined);
    ok(c.get() === undefined);
  });

  test("jashkenas/backbone#1112 - passing options.model sets collection.model", 2, function() {
    var Model = Hipbone.Model.extend({});
    var c = new Hipbone.Collection([{id: 1}], {model: Model});
    ok(c.model === Model);
  ...
  test("null and undefined are invalid ids.", 2, function() {
    var model = new Hipbone.Model({id: 1});
    var collection = new Hipbone.Collection([model]);
    model.set({id: null});
  ...
  test("jashkenas/backbone#1355 - `options` is passed to success callbacks", 2, function(){
    var m = new Hipbone.Model({x:1});
    var col = new Hipbone.Collection();
    var opts = {
      opts: true,
  ...
  test("jashkenas/backbone#1412 - Trigger 'request' and 'sync' events.", 4, function() {
    var collection = new Hipbone.Collection;
    collection.url = '/test';
    Hipbone.ajax = function(settings){ settings.success(); };
  ...
  test("jashkenas/backbone#3283 - fetch, create calls success with context", 2, function() {
    var collection = new Hipbone.Collection;
    collection.url = '/test';
  ...
  test("jashkenas/backbone#1447 - create with wait adds model.", 1, function() {
    var collection = new Hipbone.Collection;
    var model = new Hipbone.Model;
  ...
  test("jashkenas/backbone#1448 - add sorts collection after merge.", 1, function() {
    var collection = new Hipbone.Collection([
      {id: 1, x: 1},
      {id: 2, x: 2}
    ]);
  ...
  test("jashkenas/backbone#1655 - groupBy can be used with a string argument.", 3, function() {
    var collection = new Hipbone.Collection([{x: 1}, {x: 2}]);
  ...
  test("jashkenas/backbone#1655 - sortBy can be used with a string argument.", 1, function() {
    var collection = new Hipbone.Collection([{x: 3}, {x: 1}, {x: 2}]);
  ...
  test("jashkenas/backbone#1604 - Removal during iteration.", 0, function() {
    var collection = new Hipbone.Collection([{}, {}]);
  ...
  test("jashkenas/backbone#1638 - `sort` during `add` triggers correctly.", function() {
    var collection = new Hipbone.Collection;
    collection.comparator = function(model) { return model.get('x'); };
    var added = [];
  ...
  test("`sort` shouldn't always fire on `add`", 1, function() {
    var c = new Hipbone.Collection([{id: 1}, {id: 2}, {id: 3}], {
      comparator: 'id'
    });
  ...
  test("jashkenas/backbone#1407 parse option on constructor parses collection and models", 2, function() {
    var model = {
      namespace : [{id: 1}, {id:2}]
    };
    var Collection = Hipbone.Collection.extend({
  ...
  test("jashkenas/backbone#1407 parse option on reset parses collection and models", 2, function() {
    var model = {
      namespace : [{id: 1}, {id:2}]
    };
    var Collection = Hipbone.Collection.extend({
  ...
  test("set", function() {
    var m1 = new Hipbone.Model();
    var m2 = new Hipbone.Model({id: 2});
    var m3 = new Hipbone.Model();
    var c = new Hipbone.Collection([m1, m2]);
  ...
  test("set + merge with default values defined", function() {
    ...
    var col = new Hipbone.Collection([m], {model: Model});
  ...
  test('merge without mutation', function () {
    ...
    var Collection = Hipbone.Collection.extend({model: Model});
  ...
  test('`set` matches input order in the absence of a comparator', function () {
    var one = new Hipbone.Model({id: 1});
    var two = new Hipbone.Model({id: 2});
    var three = new Hipbone.Model({id: 3});
    var collection = new Hipbone.Collection([one, two, three]);
  ...
  test("jashkenas/backbone#1894 - Push should not trigger a sort", 0, function() {
    var Collection = Hipbone.Collection.extend({
      comparator: 'id',
  ...
  test("jashkenas/backbone#2428 - push duplicate models, return the correct one", 1, function() {
    var col = new Hipbone.Collection;
  ...
  test("`set` with non-normal id", function() {
    var Collection = Hipbone.Collection.extend({
      model: Hipbone.Model.extend({idAttribute: '_id'})
    });
  ...
  test("jashkenas/backbone#1894 - `sort` can optionally be turned off", 0, function() {
    var Collection = Hipbone.Collection.extend({
      comparator: 'id',
      sort: function() { ok(false); }
    });
  ...
  test("jashkenas/backbone#1915 - `parse` data in the right order in `set`", function() {
    var collection = new (Hipbone.Collection.extend({
      parse: function (data) {
  ...
  asyncTest("jashkenas/backbone#1939 - `parse` is passed `options`", 1, function () {
    var collection = new (Hipbone.Collection.extend({
      url: '/',
  ...
  test("fetch will pass extra options to success callback", 1, function () {
    var SpecialSyncCollection = Hipbone.Collection.extend({
      url: '/test',
      sync: function (method, collection, options) {
        _.extend(options, { specialSync: true });
        return Hipbone.Collection.prototype.sync.call(this, method, collection, options);
      }
    });
  ...
  test("jashkenas/backbone#2606 - Collection#create, success arguments", 1, function() {
    var collection = new Hipbone.Collection;
    collection.url = 'test';
    collection.create({}, {
  ...
  test("jashkenas/backbone#2612 - nested `parse` works with `Collection#set`", function() {

    var Job = Hipbone.Model.extend({
      constructor: function() {
        this.items = new Items();
        Hipbone.Model.apply(this, arguments);
      },
    ...
    var Item = Hipbone.Model.extend({
      constructor: function() {
        this.subItems = new Hipbone.Collection();
        Hipbone.Model.apply(this, arguments);
      },
      parse: function(attrs) {
    ...
    var Items = Hipbone.Collection.extend({
      model: Item
    });
  ...
  test('_addReference binds all collection events & adds to the lookup hashes', 9, function() {
    ...
    var Collection = Hipbone.Collection.extend({
      ...
      _addReference: function(model) {
        Hipbone.Collection.prototype._addReference.apply(this, arguments);
        calls.add++;
      ...
      _removeReference: function(model) {
        Hipbone.Collection.prototype._removeReference.apply(this, arguments);
        calls.remove++;
  ...
  test('jashkenas/backbone#3020: #set with {add: false} should not throw.', 2, function() {
    var collection = new Hipbone.Collection;
  ...
  test("create with wait, model instance, jashkenas/backbone#3028", 1, function() {
    var collection = new Hipbone.Collection();
    var model = new Hipbone.Model({id: 1});
    model.sync = function(){
      equal(this.collection, collection);
  ...
  test("modelId", function() {
    var Stooge = Hipbone.Model.extend();
    var StoogeCollection = Hipbone.Collection.extend({model: Stooge});
  ...
  test('Polymorphic models work with "simple" constructors', function () {
    var A = Hipbone.Model.extend();
    var B = Hipbone.Model.extend();
    var C = Hipbone.Collection.extend({
  ...
  test('Polymorphic models work with "advanced" constructors', function () {
    var A = Hipbone.Model.extend({idAttribute: '_id'});
    var B = Hipbone.Model.extend({idAttribute: '_id'});
    var C = Hipbone.Collection.extend({
      model: Hipbone.Model.extend({
  ...
  test("jashkenas/backbone#3039: adding at index fires with correct at", 3, function() {
    var col = new Hipbone.Collection([{at: 0}, {at: 4}]);
  ...
  test("jashkenas/backbone#3039: index is not sent when at is not specified", 2, function() {
    var col = new Hipbone.Collection([{at: 0}]);
    col.on('add', function(model, col, options) {
  ...
  test('jashkenas/backbone#3199 - Order changing should trigger a sort', 1, function() {
      var one = new Hipbone.Model({id: 1});
      var two = new Hipbone.Model({id: 2});
      var three = new Hipbone.Model({id: 3});
      var collection = new Hipbone.Collection([one, two, three]);
  ...
  test('jashkenas/backbone#3199 - Adding a model should trigger a sort', 1, function() {
    var one = new Hipbone.Model({id: 1});
    var two = new Hipbone.Model({id: 2});
    var three = new Hipbone.Model({id: 3});
    var collection = new Hipbone.Collection([one, two, three]);
  ...
  test('jashkenas/backbone#3199 - Order not changing should not trigger a sort', 0, function() {
    var one = new Hipbone.Model({id: 1});
    var two = new Hipbone.Model({id: 2});
    var three = new Hipbone.Model({id: 3});
    var collection = new Hipbone.Collection([one, two, three]);
  ...
  test("jashkenas/backbone#set accepts options.at as a string", 1, function() {
    var collection = new Hipbone.Collection([{id: 1}, {id: 2}]);
  ...
  test("removing models triggers `update` event once", 1, function() {
    var collection = new Hipbone.Collection([{id: 1}, {id: 2}, {id: 3}]);
    collection.on('update', function() { ok(true); });
  ...
  test("jashkenas/backbone#3610 - invoke collects arguments", 3, function() {
    var Model = Hipbone.Model.extend({
