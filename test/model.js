(function() {
  ...
  var proxy = Hipbone.Model.extend();
  var klass = Hipbone.Collection.extend({
    url : function() { return '/collection'; }
  });
  var doc, collection;

  module("Hipbone.Model", {
  ...
  test("underscore methods", 5, function() {
    var model = new Hipbone.Model({ 'foo': 'a', 'bar': 'b', 'baz': 'c' });
  ...
  test("chain", function() {
    var model = new Hipbone.Model({ a: 0, b: 1, c: 2 });
    deepEqual(model.chain().pick("a", "b", "c").values().compact().value(), [1, 2]);
  });

  test("clone", 10, function() {
    var a = new Hipbone.Model({ 'foo': 1, 'bar': 2, 'baz': 3});
    var b = a.clone();
    equal(a.get('foo'), 1);
    equal(a.get('bar'), 2);
    equal(a.get('baz'), 3);
    ...
    var foo = new Hipbone.Model({p: 1});
    var bar = new Hipbone.Model({p: 2});
  ...
  test("isNew", 6, function() {
    ...
    a = new Hipbone.Model({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': -5 });
    ok(!a.isNew(), "any defined ID is legal, negative or positive");
    a = new Hipbone.Model({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': 0 });
    ok(!a.isNew(), "any defined ID is legal, including zero");
    ok( new Hipbone.Model({          }).isNew(), "is true when there is no id");
    ok(!new Hipbone.Model({ 'id': 2  }).isNew(), "is false for a positive integer");
    ok(!new Hipbone.Model({ 'id': -5 }).isNew(), "is false for a negative integer");
  });
  ...
  test("matches with predicate", function() {
    var model = new Hipbone.Model({a: 0});
  ...
  test("set and unset", 8, function() {
    var a = new Hipbone.Model({id: 'id', foo: 1, bar: 2, baz: 3});
    var changeCount = 0;
  ...
  test("set falsy values in the correct order", 2, function() {
    var model = new Hipbone.Model({result: 'result'});
  ...
  test("set an empty string", 1, function() {
    var model = new Hipbone.Model({name : "Model"});
    model.set({name : ''});
  ...
  test("clear", 3, function() {
    var changed;
    var model = new Hipbone.Model({id: 1, name : "Model"});
  ...
  test("defaults", 4, function() {
    var Defaulted = Hipbone.Model.extend({
      defaults: {
        "one": 1,
        "two": 2
      }
    ...
    Defaulted = Hipbone.Model.extend({
      defaults: function() {
        return {
          "one": 3,
          "two": 4
  ...
  test("change, hasChanged, changedAttributes, previous, previousAttributes", 9, function() {
    var model = new Hipbone.Model({name: "Tim", age: 10});
  ...
  test("changedAttributes", 3, function() {
    var model = new Hipbone.Model({a: 'a', b: 'b'});
    deepEqual(model.changedAttributes(), false);
  ...
  test("change with options", 2, function() {
    var value;
    var model = new Hipbone.Model({name: 'Rob'});
  ...
  test("change after initialize", 1, function () {
    ...
    var obj = new Hipbone.Model(attrs);
  ...
  test("save within change event", 1, function () {
    var env = this;
    var model = new Hipbone.Model({firstName : "Taylor", lastName: "Swift"});
    model.url = '/test';
    model.on('change', function () {
      model.save();
  ...
  test("validate after save", 2, function() {
    var lastError, model = new Hipbone.Model();
  ...
  test("save will pass extra options to success callback", 1, function () {
    var SpecialSyncModel = Hipbone.Model.extend({
  ...
  test("destroy", 3, function() {
    ...
    var newModel = new Hipbone.Model;
  ...
  test("validate on unset and clear", 6, function() {
    var error;
    var model = new Hipbone.Model({name: "One"});
    model.validate = function(attrs) {
      if (!attrs.name) {
        error = true;
        return "No thanks.";
  ...
  test("defaults always extend attrs (jashkenas/backbone#459)", 2, function() {
    var Defaulted = Hipbone.Model.extend({
      defaults: {one: 1},
  ...
  test("Inherit class properties", 6, function() {
    var Parent = Hipbone.Model.extend({
  ...
  test("hasChanged/set should use same comparison", 2, function() {
    var changed = 0, model = new Hipbone.Model({a: null});
    model.on('change', function() {
  ...
  test("jashkenas/backbone#582, jashkenas/backbone#425, change:attribute callbacks should fire after all changes have occurred", 9, function() {
    var model = new Hipbone.Model;
  ...
  test("jashkenas/backbone#871, set with attributes property", 1, function() {
    var model = new Hipbone.Model();
  ...
  test("set same value does not trigger change", 0, function() {
    var model = new Hipbone.Model({x: 1});
  ...
  test("unset does not fire a change for undefined attributes", 0, function() {
    var model = new Hipbone.Model({x: undefined});
  ...
  test("`previous` for falsey keys", 2, function() {
    var model = new Hipbone.Model({0: true, '': true});
  ...
  test("`save` with `wait` sends correct attributes", 5, function() {
    var changed = 0;
    var model = new Hipbone.Model({x: 1, y: 2});
    model.url = '/test';
  ...
  test("jashkenas/backbone#1030 - `save` with `wait` results in correct attributes if success is called during sync", 2, function() {
    var model = new Hipbone.Model({x: 1, y: 2});
    model.sync = function(method, model, options) {
      options.success();
    };
  ...
  test("jashkenas/backbone#1122 - clear does not alter options.", 1, function() {
    var model = new Hipbone.Model();
    var options = {};
    model.clear(options);
    ok(!options.unset);
  ...
  test("jashkenas/backbone#1122 - unset does not alter options.", 1, function() {
    var model = new Hipbone.Model();
    var options = {};
  ...
  test("jashkenas/backbone#1355 - `options` is passed to success callbacks", 3, function() {
    var model = new Hipbone.Model();
    var opts = {
  ...
  test("jashkenas/backbone#1412 - Trigger 'sync' event.", 3, function() {
  ...
  asyncTest("jashkenas/backbone#1365 - Destroy: New models execute success callback.", 2, function() {
    new Hipbone.Model()
  ...
  test("jashkenas/backbone#1433 - Save: An invalid model cannot be persisted.", 1, function() {
    var model = new Hipbone.Model;
  ...
  test("jashkenas/backbone#1377 - Save without attrs triggers 'error'.", 1, function() {
    var Model = Hipbone.Model.extend({
      url: '/test/',
  ...
  test("jashkenas/backbone#1545 - `undefined` can be passed to a model constructor without coersion", function() {
    var Model = Hipbone.Model.extend({
      defaults: { one: 1 },
      initialize : function(attrs, opts) {
  ...
  asyncTest("jashkenas/backbone#1478 - Model `save` does not trigger change on unchanged attributes", 0, function() {
    var Model = Hipbone.Model.extend({
      sync: function(method, model, options) {
        setTimeout(function(){
          options.success();
  ...
  test("jashkenas/backbone#1664 - Changing from one value, silently to another, back to original triggers a change.", 1, function() {
    var model = new Hipbone.Model({x:1});
    model.on('change:x', function() { ok(true); });
    model.set({x:2},{silent:true});
  ...
  test("jashkenas/backbone#1664 - multiple silent changes nested inside a change event", 2, function() {
    var changes = [];
    var model = new Hipbone.Model();
    model.on('change', function() {
  ...
  test("jashkenas/backbone#1791 - `attributes` is available for `parse`", function() {
    var Model = Hipbone.Model.extend({
  ...
  test("jashkenas/backbone#1943 change calculations should use _.isEqual", function() {
    var model = new Hipbone.Model({a: {key: 'value'}});
    model.set('a', {key:'value'}, {silent:true});
    equal(model.changedAttributes(), false);
  });

  test("jashkenas/backbone#1964 - final `change` event is always fired, regardless of interim changes", 1, function () {
    var model = new Hipbone.Model();
  ...
  test("isValid", function() {
    var model = new Hipbone.Model({valid: true});
    model.validate = function(attrs) {
  ...
  test("jashkenas/backbone#1179 - isValid returns true in the absence of validate.", 1, function() {
    var model = new Hipbone.Model();
    model.validate = null;
    ok(model.isValid());
  });

  test("jashkenas/backbone#1961 - Creating a model with {validate:true} will call validate and use the error callback", function () {
    var Model = Hipbone.Model.extend({
  ...
  test("jashkenas/backbone#2034 - nested set with silent only triggers one change", 1, function() {
    var model = new Hipbone.Model();
    model.on('change', function() {
